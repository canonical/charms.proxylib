...
def test_environ_disabled():
    ...
    with mock.patch.dict(os.environ, env, clear=True):
        environ = jmodelproxylib.environ(enabled=False)
    assert environ == {}
...
def test_environ_lowercase_only():
    ...
    env = DEFAULT_ENV.copy()
    with mock.patch.dict(os.environ, env, clear=True):
        environ = jmodelproxylib.environ(enabled=True, uppercase=False)
    assert environ == {
...
def test_systemd_no_juju_app():
    ...
    with pytest.raises(jmodelproxylib.errors.JujuEnvironmentError):
        env = jmodelproxylib.environ(env, enabled=True)
        jmodelproxylib.systemd(env, "testd")
...
def test_systemd_enabled():
    ...
    with mock.patch.dict(os.environ, env, clear=True):
        ...
        content = jmodelproxylib.systemd(env, "testd")
        assert content == (
            """[Service]
...
def test_systemd_disabled():
    ...
    env["JUJU_UNIT_NAME"] = "test/0"
    with mock.patch.dict(os.environ, env, clear=True):
        env = jmodelproxylib.environ(enabled=False)
        content = jmodelproxylib.systemd(env, "testd")
        assert content == ""
...
def test_container_vars():
    ...
    env = DEFAULT_ENV.copy()
    with mock.patch.dict(os.environ, env, clear=True):
        content = jmodelproxylib.container_vars(jmodelproxylib.environ(env, enabled=True))
    assert content == [
...
def test_container_vars_no_lightkube():
    ...
    with mock.patch.dict(sys.modules, {"lightkube.models.core_v1": None}):
        ...
        for k in dict(sys.modules).keys():
            if k.startswith("jmodelproxylib"):
                sys.modules.pop(k)
        import jmodelproxylib

        importlib.reload(jmodelproxylib)

    env = DEFAULT_ENV.copy()
    with pytest.raises(NotImplementedError):
        # This should raise an error because lightkube is not available
        with mock.patch.dict(os.environ, env, clear=True):
            jmodelproxylib.container_vars(jmodelproxylib.environ(env, enabled=True))
